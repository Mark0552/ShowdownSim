...
3. **Diamond row** — flex row containing the diamond SVG and a 2-column × 3-row grid on the right:
   ...
   - **Per-side roll routing** (`oppRoll` / `myRoll` `useState` in GameBoard.tsx): `state.lastRollType` + `state.halfInning` decides which side the latest roll belongs to. `pitch` / `fielding` / `extra_base` / `steal_*` go to the fielding team; `swing` / `bunt` go to the batting team. Each side's box persists until that side rolls again — opp's pitch stays visible while I'm swinging — OR until a new at-bat starts (see clear-on-new-at-bat below).
   ...
   - **`MobileResultBox`** has three states: `awaiting` (dashed neutral border + dim "AWAITING PITCH" text, shown until the current at-bat's swing chart resolves), `good` (green tint, result favors me), `bad` (red tint, result favors opp). The `awaiting → good/bad` transition is gated by a `swingThisAtBat` boolean (set true when a `swing` roll routes; reset false on at-bat change), NOT directly by `state.lastSwingRoll` — because lastSwingRoll lingers from the previous at-bat and we want the indicator to reset on every new hitter.
