...
3. **Diamond row** — flex row containing the diamond SVG and a 2-column × 3-row grid on the right:
   ...
   - **`MobileResultBox`** stacks two lines: a small **advantage indicator** on top (`AWAITING PITCH` / `PITCHER ADV` / `BATTER ADV`) and a larger **persisted-result outcome** below (`STRIKEOUT`, `DOUBLE PLAY`, `HOME RUN!`, `STOLEN BASE`, etc.). The two have independent tints (so PITCHER ADV can be red while HOME RUN is green for the batter who hit it on the pitcher's chart). Box border tint priority: result.goodForMe (when set) > advantage good/bad > awaiting neutral.
     - **Advantage indicator behaviour** — gated by `pitchThisAtBat` (set true on the `pitch` roll, reset false on at-bat change), so it flips from AWAITING PITCH to PITCHER/BATTER ADV the moment the pitch lands — not after the swing. The pitch roll is what selects the chart (`state.usedPitcherChart`), so the advantage is decided then; waiting for the swing was a UX miss in the prior version. Resets to AWAITING PITCH on every new hitter.
     - **Persisted-result behaviour** — captured by the routing useEffect on every swing/fielding/extra-base/steal/bunt roll. **Persists across at-bat boundaries within the same half-inning** so the previous play stays visible while the new batter steps up. Cleared by the next pitch roll (so once the new at-bat's pitch lands, the advantage indicator is the only thing visible until the swing resolves) AND on half-inning flip via a half-clear effect declared **before** the routing effect — so a 3rd-out roll (which flips the half AND produces a result in the same state update) runs clear → set, queueing setStates in order, and the new result wins so the user sees what just ended the half until the new half's first pitch.
...
- **App-wide mobile pass (May 2026)** — Every page outside the in-game flow got a `(max-width: 899px)` mobile-portrait pass. Coverage: LoginPage, MainMenu, LineupsPage, TeamBuilder (+ FilterBar, CardCatalog, RosterPanel, BenchPanel, LineupBar), LobbyPage (+ SeriesCard), DraftPage (+ SetLineupScreen), StatsPage, RulesPage, SimulationPage, PricingPage. Common patterns applied: 16px+ font on text inputs (prevents iOS auto-zoom on focus); 40-48px min-height on buttons (thumb tappable); search/filter rows wrap or stack; tabs scroll horizontally when they would overflow; tables stay native with `overflow-x: auto` rather than reflowing rows (column count too high to make stacked-card layout legible); large modals (create-game, stats game-detail) become full-viewport with × icon close. **TeamBuilder** got a real architectural change: a TEAM ↔ CATALOG mobile tab toggle (`tb-mobile-tabs` div + `tb-mobile-{tab}` class on the root) since the desktop 3-column grid doesn't fit. Picking a slot in TEAM auto-switches to CATALOG; adding a card auto-switches back. The shared `--card-w` CSS variable is overridden on mobile to render 3 cards across instead of 11. Lineup bar uses `flex-wrap: wrap` on mobile so 9 batter slots wrap into 3 rows of 3. **DraftPage** stays single-column with a different strategy: the right-side roster mini moves above the pool (key reference info) via `order: 1`, the pool card grid locks to 3 cols, and the pool controls become sticky.
