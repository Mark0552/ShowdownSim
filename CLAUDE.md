...
3. **Diamond row** — flex row containing the diamond SVG and a 2-column × 3-row grid on the right:
   ...
   - **`MobileResultBox`** stacks two lines: a small **advantage indicator** on top (`AWAITING PITCH` / `PITCHER ADV` / `BATTER ADV`) and a larger **persisted-result outcome** below (`STRIKEOUT`, `DOUBLE PLAY`, `HOME RUN!`, `STOLEN BASE`, etc.). The two have independent tints (so PITCHER ADV can be red while HOME RUN is green for the batter who hit it on the pitcher's chart). Box border tint priority: result.goodForMe (when set) > advantage good/bad > awaiting neutral.
     - **Advantage indicator behaviour** — gated by `swingThisAtBat` (set true when a `swing` roll routes, reset false on at-bat change), so it resets to AWAITING PITCH on every new hitter. NOT driven directly by `state.lastSwingRoll` because that field lingers from the previous at-bat.
     - **Persisted-result behaviour** — captured by the routing useEffect on every swing/fielding/extra-base/steal/bunt roll. **Persists across at-bat boundaries** within the same half-inning, replaced when a new result lands. Cleared on half-inning flip via a half-clear effect declared **before** the routing effect — so a 3rd-out roll (which flips the half AND produces a result in the same state update) runs clear → set, queueing setStates in order, and the new result wins so the user sees what just ended the half until the new half's first swing replaces it.
