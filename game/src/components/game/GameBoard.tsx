    ...
    return (
        <div className="game-board-wrap">
            ...
            <svg viewBox="0 0 1400 950" className="game-board-svg">
                ...
                {/* ====== CENTER FIELD — Rotated diamond SVG ====== */}
                ...
                {/* Diamond SVG scaled to fit center field: scale(0.419) with offset */}
                <g transform="translate(317,173) scale(0.419) translate(-31.455,-189.888)" clipPath="url(#centerClip)">
                ...
                {/* Runner info tags (positioned in game board coords) */}
                {runner1 && (
                    <g>
                        <rect x="900" y="840" width="110" height="30" rx="4" fill="rgba(0,0,0,0.8)" stroke="#22c55e" strokeWidth="1"/>
                        <text x="955" y="853" textAnchor="middle" fontSize="9" fill="#4ade80" fontWeight="bold" fontFamily="Arial">{runner1.name.length > 14 ? runner1.name.slice(0, 13) + '\u2026' : runner1.name}</text>
                        <text x="955" y="866" textAnchor="middle" fontSize="9" fill="#8aade0" fontWeight="bold" fontFamily="monospace">Spd: {runner1.speed}</text>
                    </g>
                )}
                {runner2 && (
                    <g>
                        <rect x="900" y="385" width="110" height="30" rx="4" fill="rgba(0,0,0,0.8)" stroke="#22c55e" strokeWidth="1"/>
                        <text x="955" y="398" textAnchor="middle" fontSize="9" fill="#4ade80" fontWeight="bold" fontFamily="Arial">{runner2.name.length > 14 ? runner2.name.slice(0, 13) + '\u2026' : runner2.name}</text>
                        <text x="955" y="411" textAnchor="middle" fontSize="9" fill="#8aade0" fontWeight="bold" fontFamily="monospace">Spd: {runner2.speed}</text>
                    </g>
                )}
                {runner3 && (
                    <g>
                        <rect x="290" y="385" width="110" height="30" rx="4" fill="rgba(0,0,0,0.8)" stroke="#22c55e" strokeWidth="1"/>
                        <text x="345" y="398" textAnchor="middle" fontSize="9" fill="#4ade80" fontWeight="bold" fontFamily="Arial">{runner3.name.length > 14 ? runner3.name.slice(0, 13) + '\u2026' : runner3.name}</text>
                        <text x="345" y="411" textAnchor="middle" fontSize="9" fill="#8aade0" fontWeight="bold" fontFamily="monospace">Spd: {runner3.speed}</text>
                    </g>
                )}

                {/* ====== CARD SLOTS centered on bases ====== */}
                {/* HP=(386,865) 3B=(395,400) 2B=(869,400) 1B=(869,855) Mound=(627,632) */}
                <CardSlot x={869 - 38} y={400 - 53} label="2B" card={runner2} labelBelow={true} labelText="2ND BASE" onHover={handlePlayerHover} onLeave={handlePlayerLeave}/>
                <CardSlot x={869 - 38} y={855 - 53} label="1B" card={runner1} labelBelow={true} labelText="1ST BASE" onHover={handlePlayerHover} onLeave={handlePlayerLeave}/>
                <CardSlot x={395 - 38} y={400 - 53} label="3B" card={runner3} labelBelow={true} labelText="3RD BASE" onHover={handlePlayerHover} onLeave={handlePlayerLeave}/>
                <CardSlot x={627 - 38} y={632 - 53} label="P" card={pitcher} onHover={handlePlayerHover} onLeave={handlePlayerLeave}/>
                <CardSlot x={386 - 38} y={865 - 53} label="H" card={batter} labelAbove={true} labelText="HITTER" onHover={handlePlayerHover} onLeave={handlePlayerLeave}/>

                {/* ====== PITCHER IP / FATIGUE ====== */}
                <rect x="672" y="690" width="90" height="22" rx="4" fill="rgba(0,0,0,0.7)"/>
                <text x="717" y="706" textAnchor="middle" fontSize="10" fill={fatigueActive ? '#ff6060' : '#8aade0'} fontWeight="bold" fontFamily="monospace">
